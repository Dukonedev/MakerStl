...
import { fetchManifest, flattenManifest, extractCategories, loadAllThumbnails, loadFullPreset, loadCustomTexture, loadPresetThumbnail }  from './presetTextures.js';
...
import { initViewNavigator } from './viewNavigator.js';
...
const UNCOLLAPSIBLE_IDS = ['section-source', 'section-export'];

function initCollapsibleSections() {
  const headers = document.querySelectorAll('.panel-section-header');

  headers.forEach(header => {
    const section = header.parentElement;
    const id = section.id;

    if (UNCOLLAPSIBLE_IDS.includes(id)) {
    ...
    if (id) {
      ...
      if (saved === 'true' || saved === null) {
        ...
        if (saved === null) localStorage.setItem(`section-collapsed-${id}`, 'true');
      } else {
        section.classList.remove('collapsed');
      }
    }
    ...
    header.addEventListener('click', () => {
      ...
      if (isCurrentlyCollapsed) {
        ...
        section.classList.add('collapsed');
        localStorage.setItem(`section-collapsed-${id}`, 'true');
      }
    });
  });
}
...
function expandSection(id) {
  if (UNCOLLAPSIBLE_IDS.includes(id)) return;
  const target = document.getElementById(id);
  if (!target) return;

  // Accordion: close others
  document.querySelectorAll('.panel-section:not(.collapsed)').forEach(other => {
    if (other !== target && !UNCOLLAPSIBLE_IDS.includes(other.id)) {
      other.classList.add('collapsed');
      if (other.id) localStorage.setItem(`section-collapsed-${other.id}`, 'true');
    }
  });

  target.classList.remove('collapsed');
  if (id) localStorage.setItem(`section-collapsed-${id}`, 'false');
}
...
initViewer(canvas);
initViewNavigator(
  document.getElementById('nav-cube-container'),
  getCamera(),
  getControls(),
  requestRender
);
...
// ── Manifest-driven dynamic presets ──────────────────────────────────────────

let IMAGE_PRESETS = [];  // populated from manifest
let _presetSwatches = [];
...
function filterByCategory(catId) {
  ...
  _presetSwatches.forEach((swatch, idx) => {
    const presetCat = IMAGE_PRESETS[idx].category;
    if (catId === 'all' || presetCat === catId) {
      swatch.classList.remove('category-hidden');
    } else {
      swatch.classList.add('category-hidden');
    }
  });
  document.querySelectorAll('.category-tab').forEach(tab => {
    tab.classList.toggle('active', tab.dataset.catId === catId);
    tab.setAttribute('aria-pressed', tab.dataset.catId === catId ? 'true' : 'false');
  });
}
...
function buildSwatches() {
  ...
  _presetSwatches = IMAGE_PRESETS.map((p, idx) => {
    const swatch = document.createElement('div');
    swatch.className = 'preset-swatch preset-loading';
    swatch.setAttribute('role', 'button');
    swatch.setAttribute('tabindex', '0');
    swatch.title = p.name;
    swatch.dataset.category = p.category;
...
// Fetch manifest & init presets
fetchManifest().then(manifest => {
  IMAGE_PRESETS = flattenManifest(manifest);
  const categories = extractCategories(manifest);

  buildSwatches();
  buildCategoryTabs(categories);

  // Load thumbnails, then auto-select default preset
  loadAllThumbnails(IMAGE_PRESETS).then(thumbs => {
    ...
    if (defaultIdx >= 0 && PRESETS[defaultIdx]) {
      selectPreset(defaultIdx, _presetSwatches[defaultIdx]);
    } else if (PRESETS.length > 0) {
      // fallback: select first available
      const firstValid = PRESETS.findIndex(p => p != null);
      if (firstValid >= 0) selectPreset(firstValid, _presetSwatches[firstValid]);
    }
  }).catch(err => console.error('Failed to load thumbnails:', err));
}).catch(err => console.error('Failed to load manifest:', err));

// ── Reload presets from manifest (used by Regenerate button) ─────────────────
...
async function reloadFromManifest() {
  ...
  const thumbs = await loadAllThumbnails(IMAGE_PRESETS);
  thumbs.forEach((thumb, idx) => {
    if (!thumb) return;
    PRESETS[idx] = thumb;
    const swatch = _presetSwatches[idx];
...
if (regenerateBtn) {
  regenerateBtn.addEventListener('click', async () => {
    console.log('Button clicked: Regenerating manifest...');
    ...
    try {
      ...
      alert('Error: Could not reach the server to regenerate.');
    } finally {
      regenerateBtn.classList.remove('busy');
    }
  });
} else {
  console.error('Error: regenerate-btn not found in DOM');
}
...
async function selectPreset(idx, swatchEl) {
  ...
  try {
    const full = await loadFullPreset(idx, IMAGE_PRESETS);
...
// (Section logic moved to the top of the file)
...
function wireEvents() {
  ...
  const _storeDismiss = document.getElementById('store-cta-dismiss');
  if (_storeDismiss) {
    _storeDismiss.addEventListener('click', () => {
  ...
  const startExport = (format) => {
    ...
    if (!overlay || sessionStorage.getItem('stlt-no-sponsor') === '1') {
      handleExport(format);
      return;
    }
    ...
    const dismiss = () => {
      const dontShow = document.getElementById('sponsor-dont-show');
    ...
    if (storeLink) storeLink.onclick = () => setTimeout(dismiss, 150);
  };
...
async function handleModelFile(file) {
  try {
    ...
    // Auto-expand Displacement Map section to guide user
    expandSection('section-map');
  } catch (err) {
