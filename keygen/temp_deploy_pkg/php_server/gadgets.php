...
try {
    ...
    if ($action === 'save') {
        ...
        $engravingDepthMm = $_POST['engravingDepthMm'] ?? $jsonData['engravingDepthMm'] ?? 0.4;
        ...
        $newGadget = [
            ...
            'engravingDepthMm' => floatval($engravingDepthMm),
    ...
    if ($action === 'toggleVisibility') {
        ...
        foreach ($gadgets as $k => $g) {
            if ($g['id'] === $id) {
                $gadgets[$k]['hidden'] = !($g['hidden'] ?? false);
                $found = true;
                break;
            }
        }
        ...
        if ($found) {
            file_put_contents($dataFile, json_encode($gadgets, JSON_PRETTY_PRINT));
